# Correo a los ponentes confirmados — especificaciones y documentación

**Para:** los 7 ponentes confirmados (direcciones pendientes de obtener)
**CC:** Dra. Lizbet Ruilova · Dr. Pablo Vanegas
**Adjuntos:** `SEDA_Plantilla_Ponentes.pptx` y `SEDA_Declaracion_Conflicto_Interes.docx`
**Plazo de documentación propuesto:** viernes 2 de octubre de 2026

---

Estimadas doctoras, estimados doctores:

Reciban un cordial saludo.

Les escribo en nombre de la **Sociedad de Endocrinología y Diabetes del Austro (SEDA)**, cuya coordinación académica está a cargo de la Dra. Lizbet Ruilova y el Dr. Pablo Vanegas. Estoy apoyando la organización del cuerpo docente del **Curso virtual de actualización en obesidad, diabetes, nutrición clínica y salud digital**, y les escribo porque ya confirmaron su participación.

Gracias por aceptar. Aquí tienen todo lo que necesitan para preparar su clase.

## Su clase y su fecha de entrega

El curso es virtual, con clases grabadas que se publican por módulos. **La grabación de cada módulo debe estar entregada siete días antes de su publicación**, de modo que cada quien tiene una fecha distinta:

| Ponente | Módulo · Clase | Tema | Entrega de la grabación |
|---|---|---|---|
| Lcda. Isabel Reinoso | M2 · C1 | Principios de terapia nutricional | **jueves 29 de octubre** |
| Dra. Johanna Piedra Bravo | M2 · C2 | Patrones alimentarios basados en evidencia | **jueves 29 de octubre** |
| Dr. Frank Espinoza | M3 · C2 | Agonistas del receptor de GLP-1 | jueves 5 de noviembre |
| Dra. Teresa Cuatecontzi | M3 · C3 | Tirzepatida y nuevas terapias | jueves 5 de noviembre |
| Dra. Josefa Palacio Riofrío | M4 · C2 | Monitoreo glucémico y metas terapéuticas | jueves 12 de noviembre |
| Dr. Chih Hao Chen Ku | M6 · C2 | Riesgo cardiovascular y dislipidemias | jueves 26 de noviembre |
| Dra. Adriana Alvarez | M6 · C4 | MASLD, sarcopenia y complicaciones metabólicas | jueves 26 de noviembre |

El curso inicia el **jueves 5 de noviembre** y cierra el **jueves 17 de diciembre**.

## Especificaciones de la presentación

- **Duración: 30 minutos** de grabación. Como referencia, entre 18 y 22 diapositivas de contenido.
- **Use la plantilla institucional adjunta.** Mantiene la identidad visual del curso en las 28 clases.
- **Tres diapositivas son obligatorias:** portada, datos del ponente y declaración de conflicto de interés.
- **Incluya un caso clínico** articulado con sus objetivos de aprendizaje, y anonimizado: sin nombres, iniciales, fechas exactas ni números de historia clínica.
- **Bibliografía en formato Vancouver**, únicamente con fuentes reales e indexadas y con datos de cita completos. Cada referencia se verifica antes de presentar el expediente.
- **Emplee la denominación genérica de los medicamentos**, sin marcas comerciales.

La primera diapositiva de la plantilla contiene estas instrucciones y debe eliminarse antes de grabar.

## Documentación que necesito de cada uno

Estamos preparando la solicitud de aval académico ante la **Universidad de Cuenca**, y el expediente exige documentación de cada docente. Les pido, por favor, lo siguiente:

**1. Declaración de conflicto de interés firmada.** Va adjunta. Es obligatoria para todos, incluso si no tienen ningún vínculo que declarar. El programa aborda agonistas de GLP-1, tirzepatida e iSGLT2, y el curso contempla auspicio externo; la transparencia los protege a ustedes y respalda la solidez del aval.

**2. Hoja de vida resumida.** Ya tengo las de la **Dra. Piedra, la Dra. Palacio y el Dr. Chen Ku** — muchas gracias. Me faltan las de la **Lcda. Reinoso, el Dr. Espinoza, la Dra. Cuatecontzi y la Dra. Alvarez.**

**3. Número de cédula o pasaporte.**

**4. Una fotografía profesional.**

**5. El título definitivo de su charla**, si desean ajustar el que aparece en el cuadro.

Les agradecería recibir todo antes del **viernes 2 de octubre**. Es la fecha que nos permite presentar el expediente a tiempo.

## Dos aclaraciones

**Sobre el formato de la hoja de vida:** envíenla en el formato que tengan. Si la Universidad exige uno específico, se los haré saber y les ayudaré a adaptarla.

**Sobre el aval:** está en trámite ante la Universidad de Cuenca. Les confirmaré en cuanto haya resolución.

Cualquier duda, con toda confianza. Gracias por sumarse a este proyecto.

Cordialmente,

**Dra. Omidres Pérez de Carvelli**
Endocrinología · Medicina Interna
En apoyo a la coordinación académica de SEDA
draomidresperez.com
